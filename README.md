...
- [Redis](https://www.drupal.org/project/redis) 2.x. That branch has no stable
  release yet, so the constraint has to carry the stability flag: a plain
  `composer require drupal/redis` installs 1.11.0, on which this module cannot
  run, and `drupal/redis:^2.0` does not resolve under the default
  `minimum-stability`. Install it with:

  ```
  composer require "drupal/redis:^2.0@alpha"
  ```

- The [phpredis](https://github.com/phpredis/phpredis) PHP extension, 5.3 or
  newer, for the client this module configures. The redis module's Predis
  client also works, without the extension: the two optimisations that depend
  on Lua fall back to the commands the redis module sends, and the status
  report says so. That is why `composer.json` suggests `ext-redis` rather than
  requiring it.

Drupal 10.3 or later, or 11.4 or later. In 11.2 core added a cache tag preload
of its own, which collided with this module's until the module was changed to
keep out of its way. That fix was verified on 11.4.6 and the range was set to
match, so Drupal 11.0 to 11.3 are not claimed.
...
With `opcache.validate_timestamps=0`, the usual production setting, rebuilding
caches is not enough: PHP-FPM goes on running the `settings.php` it compiled
until it is reloaded, while `drush` is a process of its own and reads the file
as it is on disk. Measured on PHP-FPM with that setting: after a change to
`settings.php`, web requests kept the old configuration through five seconds and
through a `drush cr` that read the new one, and picked it up on the first
request after the pool was reloaded (`SIGUSR2` to the master process). Reload
PHP-FPM after every change here.
...
Without it those three fields say `off`, not `0`. It counts a `PhpRedisRtt`
connection and does nothing on the redis module's own clients; for those, see
Rolling out. They are separate because they cost different things, but neither
is free.
...
The header and its counter both belong to this module, so an instance without
the module sends nothing to compare against. The baseline has the module
installed and none of its settings in place - which on its own changes nothing -
plus `$settings['redis_rtt_report'] = TRUE` and
`$settings['redis.connection']['interface'] = 'CountingPhpRedis'`. That is the
redis module's own PhpRedis connection, established exactly as that module
establishes it, wrapped in the same counter. It counts without
`count_commands`, and the redis module's backend, checksums and locks stay as
they are.
...
Each stage is reverted by undoing its own lines. The one that is easy to get
wrong is the connection: `interface` names a client whose factory this module's
services file defines, so it goes before, or together with, that
`container_yamls` line - see Recovery. Cache entries have the same format as the
stock backend's, so nothing persists in an incompatible state.
...
landed. With `opcache.validate_timestamps=0`, reload PHP-FPM before making it
(see Configuration): until then the request is answered with the previous
stage's `settings.php`, whatever the new one does.
...
because nothing has been written anywhere yet. If instead the module's directory
was removed while `settings.php` still names its classes, see **Nothing works
and you want out now** below.
...
`$settings['cache']['default'] = 'cache.backend.redis'`, comment out the
`interface` line and the two `redis_rtt` `container_yamls` lines, **put the
redis module's own `example.services.yml` back in their place** if it is not
already listed, and rebuild caches:
...
If `$settings['bootstrap_container_definition']` is in use, set its
`redis.factory` class back to `Drupal\redis\ClientFactory` as well. With
`opcache.validate_timestamps=0`, reload PHP-FPM before checking the result.

The `interface` line decides whether you are actually out. It names a client
whose factory is defined in this module's services file, so it has to go with
that file. Measured with everything else in the recipe applied: with the module
still installed, Drupal loads that services file on its own, the site answers
200 and the redis module's backend, checksums and locks are back - but the
client is still `PhpRedisRtt`, with this module's read timeout. With the module
uninstalled, the site went on serving until the cache rebuild, `drush cr`
stopped with `Invalid interface PhpRedisRtt`, and from then on every page was a
500 and `drush` would not boot. With the line commented out the client is
`PhpRedis` in both cases.

Once the module is uninstalled and the recipe is in place, its directory can be
removed: measured, the site still answers 200 on `PhpRedis` and `drush cr` still
works. The one thing that must not still point at it is the bootstrap container.
Left naming `Drupal\redis_rtt\ClientFactory`, removing the directory made every
page a 500 and `drush` would not boot, until the class was set back.

The `example.services.yml` line is the one that is easy to miss and the one that
decides whether this is free. `redis_rtt.services.example.yml` is where the cache tag checksum
...
by four conditions, and it never reproduced outside a deliberately misconfigured
one.
...
`settings.php`, or its path does not match where the module actually lives. If
the module's directory has just been removed, `bootstrap_container_definition`
still names this class: set its `redis.factory` back to
`Drupal\redis\ClientFactory`; see Recovery.

**`Invalid interface PhpRedisRtt`.**
`$settings['redis.connection']['interface']` names this module's client, and
the factory building the connection does not know it. There are two ways to get here, with opposite fixes:

- **Setting the module up.** `$settings['bootstrap_container_definition']`
  names the redis module's own `Drupal\redis\ClientFactory`. Point it at
  `Drupal\redis_rtt\ClientFactory`; see Configuration.
- **Taking the module out.** It is uninstalled, its `container_yamls` lines are
  commented out, and the `interface` line is still there. Comment that line out
  too; see Recovery. Pointing the bootstrap container at
  `Drupal\redis_rtt\ClientFactory` does not help here: measured, the site stays
  a 500 with the same message, because the factory that throws is the main
  container's, which lost this module's client along with its services file.
...
**Nothing seems faster.** Check `X-Redis-RTT` with `redis_rtt_report` and
`$settings['redis.connection']['count_commands']` both enabled. With only the
first, the three Redis fields say `off`; if they still say it with both, the
connection is not `PhpRedisRtt`, or PHP-FPM has not been reloaded since the
change. If round trips dropped but wall time did not, the network is not your
