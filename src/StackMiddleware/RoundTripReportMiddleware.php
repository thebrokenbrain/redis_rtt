 ...
 *
 * Enable with $settings['redis_rtt_report'] = TRUE. The three Redis fields are
 * filled in only when the connection is counted as well - count_commands on a
 * PhpRedisRtt connection, or the CountingPhpRedis interface - and say "off"
 * otherwise. Keep it off by default: the database query log holds every query
...
class RoundTripReportMiddleware implements HttpKernelInterface {
  ...
  protected function report(): array {
    // "off" rather than 0 when nothing is being counted. The Redis fields come
    // from CountingClient, which is a separate switch, and a 0 next to real
    // db-queries reads as a site that is not talking to Redis at all - the
    // very conclusion the header was switched on to rule out.
    $parts = CountingClient::$instrumented ? [
      'redis-trips=' . CountingClient::$roundTrips,
      'redis-cmds=' . CountingClient::$commands,
      'redis-ms=' . round(CountingClient::$nanoseconds / 1e6, 1),
    ] : ['redis-trips=off', 'redis-cmds=off', 'redis-ms=off'];
