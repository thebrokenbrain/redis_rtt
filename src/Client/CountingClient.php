...
class CountingClient implements ClientInterface {
  ...
  /**
   * Whether any connection has been wrapped in this counter.
   *
   * What lets the report tell "counted nothing" from "was not counting".
   * Without it, a site with redis_rtt_report on and count_commands off sent
   * redis-trips=0 next to real database figures, which reads as a site that
   * does not talk to Redis at all.
   *
   * ::reset() leaves it alone on purpose. The report middleware resets the
   * counters as it starts, and by then the connection has usually been built:
   * the page cache middleware, which runs inside it, takes its cache bin as a
   * constructor argument, and the bin takes its client. Clearing this there
   * would report a live counter as off. It cannot outlast the request, since
   * PHP starts every request with its statics at their defaults.
   */
  public static bool $instrumented = FALSE;
